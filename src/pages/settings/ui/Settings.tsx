...
import { Button, Card, Panel, Progress } from "@/shared/ui/Primitives";
...
export default function Settings() {
  ...
  const { reset, signOut, user, userLevel, userXP, xpToNextLevel, currencyInfo, setCurrencyInfo, currencyRates } = useUserStore();
  ...
  const handleRegionChange = (countryCode: string) => {
    if (countryCode === "KZ") {
      setCurrencyInfo({ code: "KZT", symbol: "₸", rateToUSD: currencyRates["KZT"] || 450, countryCode: "KZ" });
    } else if (countryCode === "RU") {
      setCurrencyInfo({ code: "RUB", symbol: "₽", rateToUSD: currencyRates["RUB"] || 92, countryCode: "RU" });
    } else if (countryCode === "UA") {
      setCurrencyInfo({ code: "UAH", symbol: "₴", rateToUSD: currencyRates["UAH"] || 40, countryCode: "UA" });
    } else {
