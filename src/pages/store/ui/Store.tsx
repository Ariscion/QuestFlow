...
import { Button, Card, Panel, Pill, Skeleton } from "@/shared/ui/Primitives";
import { GameCover } from "@/entities/game/ui/GameCover";
import { useDeals } from "@/entities/game/model/useDeals";
                                        ...
                                        {(deal.steamRatingPercent || deal.metacriticScore) && (
                                            <div className="flex flex-wrap justify-center sm:justify-start gap-2 mt-2">
                                                {deal.steamRatingPercent && (
                                                    <span className="text-[10px] text-emerald-400 font-extrabold bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full" title={`${deal.steamRatingText || 'Positive'} (${deal.steamRatingCount ? Number(deal.steamRatingCount).toLocaleString('ru-RU') : ''} отзывов)`}>
                                                        👍 {deal.steamRatingPercent}%
                                                    </span>
                                                )}
                                                {deal.metacriticScore && (
                                                    <span className="text-[10px] text-yellow-400 font-extrabold bg-yellow-500/10 border border-yellow-500/20 px-2 py-0.5 rounded-full">
                                                        Ⓜ️ {deal.metacriticScore}
                                                    </span>
                                                )}
                                            </div>
                                        )}
                                        <p className="text-xs text-emerald-400 mt-2 uppercase tracking-wide font-semibold">
