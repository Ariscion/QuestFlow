...
import { logoutUser, saveUserDataToDb } from "@/shared/lib/firebase";
import { useUserStore } from "@/entities/user/model/userStore";
import { IconBell, IconDownloads, IconHelp, IconHome, IconLibrary, IconSettings, IconStore } from "@/shared/ui/icons";
        ...
        <div className="h-[100dvh] w-full flex items-center justify-center sm:p-6 bg-slate-950 sm:bg-transparent overflow-hidden">
            <div className="w-full h-full sm:w-[1180px] sm:max-w-[96vw] sm:h-[720px] sm:max-h-[92vh] flex flex-col-reverse sm:flex-row overflow-hidden sm:rounded-3xl sm:border border-slate-800 bg-slate-900 shadow-xl relative">
                ...
                {loc.pathname !== "/store" && (
                    <button
                        onClick={() => nav("/store")}
                        className="sm:hidden fixed bottom-[88px] right-6 w-14 h-14 bg-blue-600 hover:bg-blue-500 rounded-full shadow-[0_4px_20px_rgba(16,185,129,0.5)] flex items-center justify-center z-50 text-white transition-transform active:scale-95 border border-white/10"
